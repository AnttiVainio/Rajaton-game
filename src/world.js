...
function World(gl, shader, alphaShader, colorShader, textShader, LEVEL_PREV, TRUE_LEVEL, EFFECTIVE_LEVEL, fluid) {
    const TILES = 9; // Amount of tilesets

    const BASE_LIGHT_FG = 1.4;
    const BASE_LIGHT_BG = 0.25;
    const LIGHT_MULT = 6;

    const SPECIAL_RANDOM_MAX = 8;

    let _tile = 1;
    const maxtile = Math.floor(EFFECTIVE_LEVEL < 16 ? EFFECTIVE_LEVEL / 4 : (EFFECTIVE_LEVEL + 3) / 5) + 1;
    if (maxtile > 1) {
        if (maxtile > TILES && !randInt(0, 50)) _tile = TILES + 1;
        else if (maxtile > TILES + 1 && !randInt(0, 100)) _tile = TILES + 2;
        else _tile = randInt(1, Math.min(TILES, maxtile));
    }
    if (TRUE_LEVEL === 3) _tile = 2;

    const fadeBox = new BoxTex(gl, alphaShader, "fade");
    const lampBox = new BoxTex(gl, alphaShader, "lamp");
    const glowBox = new BoxTex(gl, shader, "particle").setAdditive();
    ...
    let ARENA_TIME = 30;
    const FADE_COLOR = 1;
    let SIZEX = Math.min(TRUE_LEVEL <= 2 ? 60 : 60 + (EFFECTIVE_LEVEL < 25 ? EFFECTIVE_LEVEL * 2 : 50 + EFFECTIVE_LEVEL - 25), 180);
    let SIZEY = Math.min(TRUE_LEVEL <= 2 ? 15 : 15 + (EFFECTIVE_LEVEL < 25 ? EFFECTIVE_LEVEL     : 25 + Math.floor((EFFECTIVE_LEVEL - 25) / 3)), 60);

    // Randomize world size sometimes
    if (TRUE_LEVEL > 3) {
        if (!randInt(0, SPECIAL_RANDOM_MAX)) {
            const temp = SIZEX;
            SIZEX = Math.max(50, Math.round(SIZEY * 1.5));
            SIZEY = Math.round(temp / 1.5);
        }
        else if (!randInt(0, SPECIAL_RANDOM_MAX - 1)) {
            SIZEX *= 2;
            SIZEY = Math.round(SIZEY / 2);
        }
    }
    ...
    function fluidType() {
        if (EFFECTIVE_LEVEL < 6) return FLUID_WATER;
        return Math.random() > Math.max(0.55, 0.35 + Math.pow(0.7, Math.sqrt(EFFECTIVE_LEVEL - 2))) ? FLUID_LAVA : FLUID_WATER;
    }
    ...
    // Only one shaft in level 1, quaranteed liquid, no arena, no other fluid, extra lamp at spawn
    {
        ...
        const MAXX = SIZEX - 1;
        let X1 = 1;
        let H = randInt(2, 3); // starting tunnel height
        let Y = TRUE_LEVEL === 1 ? SIZEY - 1 - H : randInt(1, SIZEY - 1 - H);
        starty = Y + H - 1;
        let vertical = false;

        // Dead ends randomization
        const DEAD_ENDS = randInt(1, 3);
        const DEAD_END_MIN = randInt(1, 2);
        const DEAD_END_MAX = randInt(2, 5);
        const DEAD_END_MAX_2 = randInt(-2, 2);

        // Branching dead end
        const branch = (v1, v2, v3, dx, dy) => {
            if (randInt(0, DEAD_ENDS)) {
                const W = randInt(DEAD_END_MIN, DEAD_END_MAX);
                if (v2 - W - v1 > 1) {
                    const L = randInt(W + 1, W * 2 + 2 + DEAD_END_MAX_2);
        ...
        // World generation randomization
        let SHAFTS = randInt(0, 6);
        let SHAFT_MAX = 4;
        let W_MIN = randInt(1, 2);
        let W_MAX = randInt(-1, 2);
        let H_MIN = randInt(1, 2);
        let H_MAX = randInt(-2, 2);
        if (TRUE_LEVEL > 3) {
            if (!randInt(0, SPECIAL_RANDOM_MAX)) {
                SHAFTS = 10;
            }
            if (!randInt(0, SPECIAL_RANDOM_MAX)) {
                SHAFT_MAX += randInt(1, 2);
            }
            if (!randInt(0, SPECIAL_RANDOM_MAX)) {
                W_MIN = 1;
                W_MAX = -2;
        ...
        const SHAFT_EMITTERS = randInt(2, 4);
        const W_MAX_2 = randInt(-3, 3);
        const H_MAX_2 = randInt(-3, 3);
        let LAMPS = rand(0.7, 1.3);
        let FLUIDS = rand(0.7, 1.3);
        let SMALL_FLUIDS = false;
        if (TRUE_LEVEL > 3) {
            if (!randInt(0, SPECIAL_RANDOM_MAX)) {
                LAMPS *= rand(2, 4);
            }
        ...
        while (X1 < MAXX) {
            if (vertical) {
                ...
                const shaft = TRUE_LEVEL !== 1 && !arena && !randInt(0, SHAFTS); // build shaft?
                const W = TRUE_LEVEL !== 1 && arena ? randInt(9, 11) : Math.min(X1 - 1, randInt(W_MIN, (shaft ? randInt(3,SHAFT_MAX) : randInt(4,5)) + W_MAX));
                const H2 = randInt(H_MIN, randInt(4,5) + H_MAX);

                // create arena
                if (TRUE_LEVEL % 2 === 0 && arena) {
                    ...
                    // arena solids
                    let twoSolids = 0;
                    if (randInt(0, 1)) { // two solids next to each other
                        twoSolids = 2;
                    ...
                    for (let i = 0; i < randInt(2, 4) - twoSolids; i++) {
                        let x = 0;
                        let y = 0;
                ...
                // create shaft
                else if ((TRUE_LEVEL === 1 && arena) || shaft) {
                    shaftx = X1 - W;
                    shafty = Y + H;
                    ...
                    if (TRUE_LEVEL === 1 || !randInt(0, SHAFT_EMITTERS)) fluid.addEmitter(randInt(X1 - W, X1 - 1), 1, fluidType());
                ...
                // normal path
                    ...
                    if (dir) Y = Math.min(SIZEY - 1 - H2, Y + randInt(W + 1, W * 2 + 3 + W_MAX_2));
                    else Y = Math.max(1, Y - randInt(W + 1, W * 2 + 3 + W_MAX_2));
            ...
            // horizontal path
                ...
                const X2 = Math.max(14, Math.min(MAXX, X1 + randInt(H + 3, H * 2 + 8 + H_MAX_2) + (X1 === 1 ? 4 : 0)));
        ...
        for (let i = 0; i < Math.max(1, Math.round(SIZEX * 0.066 * LAMPS)); i++) {
            while (true) {
        ...
        if (TRUE_LEVEL !== 1) {
            for (let i = 0; i < Math.round(SIZEX * 0.025 * FLUIDS); i++) {
                while (true) {
                    const x = randInt(1, SIZEX - 2);
                    let y = randInt(1, SIZEY - 2);
                    while (!getWorld(x, y - 1).solid) y--;
                    ...
                    if (!getWorld(x, y).shaft && !getWorld(x, y).door && !getWorld(x, y).solid && (getWorld(x, y).arena || getWorld(x, y).branch)) {
                        ...
                        // for map rendering
                        if (type == FLUID_WATER) getWorld(x, y).water = true;
                        else getWorld(x, y).lava = true;
                        fluid.addEmitter(x, y, type, getWorld(x, y).arena || SMALL_FLUIDS);
                        break;
                    }
                }
            }
    ...
    this.getArena = function(playerx) {
        if (arenaTime < ARENA_TIME) {
            ...
            if (arenaTime < 0 && playerx > arenax) {
                arenaTime = 0;
                for (const i of arenaTiles) i.solid = true;
            }
    ...
    this.act = function(delta) {
        ...
        if (arenaTime > ARENA_TIME && arenaTiles) {
            for (const i of arenaTiles) i.solid = false;
            arenaTiles = null;
        }
    }
    ...
    const finalUnlock = EFFECTIVE_LEVEL >= FINAL_UNLOCK && LEVEL_PREV < FINAL_UNLOCK;
    const finalUnlockText = EFFECTIVE_LEVEL >= FINAL_UNLOCK_TEXT && LEVEL_PREV < FINAL_UNLOCK_TEXT;
    let name = ["LEVEL", "LAYER", "FLOOR", "SHAFT", "CHASM", "RIFT", "RUIN", "ABYSS", "PASSAGE", "TRANSIT", "PATHWAY", "DOMAIN", "ZONE", "SECTOR"];
    name = name[randInt(0, name.length - 1)];
    if (!randInt(0, 10)) {
        name = ["HELL", "BLIGHT", "DOOM", "PERDITION"];
        name = name[randInt(0, name.length - 1)];
    }
    if (finalUnlock) name = "THE END";
    ...
    this.draw = function() {
        ...
        for (let y = Math.max(0, Math.floor((cameray - 0.9 * INVERSE_ASPECT) * drawMult)); y < y2; y++) {
            for (let x = Math.max(0, Math.floor((camerax - 0.9) * drawMult)); x < x2; x++) {
                const tile = getWorld(x, y);
                const color = (tile.solid ? BASE_LIGHT_FG : BASE_LIGHT_BG) + tile.lighting * LIGHT_MULT;
                ...
                if (tile.fader || tile.clock1 || tile.clock2) fadeBox.drawRot(X, Y, size, TILESIZE, PI90, FADE_COLOR);
                if (tile.fadel || tile.clock1 || tile.clock2) fadeBox.drawRot(X, Y, size, TILESIZE, -PI90, FADE_COLOR);
                // clock text
        ...
        else if (TRUE_LEVEL === EXTRA_SETTING_UNLOCK) {
        ...
        else if (finalUnlockText) {
            txt.draw("you have unlocked everything", toWorldX(TILESIZE * 8) + BUTTONSIZE * 10, toWorldY((starty - 1) * TILESIZE * 2), BUTTONSIZE, 0.9, [1, 1, 1, 0.7]);
            txt.draw("yet you still continue", toWorldX(TILESIZE * 8) + BUTTONSIZE * 10, toWorldY(starty * TILESIZE * 2), BUTTONSIZE, 0.9, [1, 1, 1, 0.7]);
        }
        else {
            txt.draw(name + "-" + TRUE_LEVEL, toWorldX(TILESIZE * 44), toWorldY((starty - 0.5) * TILESIZE * 2), TILESIZE * 2, 1.3, [1, 1, 1, 0.18], "left");
        }
    }
    ...
    this.enemyLocation = function(pos) {
        ...
        while (true) {
            ...
            if (arena) {
                ...
                x *= TILESIZE * 2;
                y *= TILESIZE * 2;
                if (Math.abs(x - pos[0]) + Math.abs(y - pos[1]) > (TRUE_LEVEL === 1 ? 4 : 2)) return [x, y];
            }
        }
    }
        ...
        for (let x = 1; x < SIZEX - 1; x++) {
            ...
            if (!tile.shaft && !tile.arena) {
                for (const l of lamps) {
                    const light = Math.min(3 / length(x * TILESIZE * 2, y * TILESIZE * 2, l.lightX(), l.lightY()) * 0.018, 0.06);
                    if (light > 0.01) {
                        if (!this.collision(x * TILESIZE * 2, y * TILESIZE * 2, l.lightX(), l.lightY())) {
                            tile.lighting += light;
                        }
                    }
                }
