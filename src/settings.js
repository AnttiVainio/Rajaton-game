...
const VERSION = "v1.1.0-2026-04-14";

const IMAGE_LOCATION = "game_images/";
const SOUND_LOCATION = "game_sounds/";

const FONT = "bold 29px monospace";
const TEXT_WIDTH = 16;

const FULLSCREEN_KEY = "KeyF";
const PAUSE_KEY = "KeyP";
const SCREENSHOT_KEY = "KeyZ";

const DEFAULT_FRAMEBUFFER_SMOOTH = false;

const MIN_FILTER = (gl, f) => [
    gl.NEAREST,                // - point filtering / nearest neighbor
    gl.LINEAR,                 // - bilinear
    gl.NEAREST_MIPMAP_NEAREST, // - point filtering / nearest neighbor (with mipmaps)
    gl.NEAREST_MIPMAP_LINEAR,  // - (default in WebGL)
    gl.LINEAR_MIPMAP_NEAREST,  // - bilinear (with mipmaps)
    gl.LINEAR_MIPMAP_LINEAR,   // - trilinear
][f];

const TEXT_FILTER = 2; // MIN filter
const TEXT_SMOOTH = false; // MAG filter
const TEXTURE_FILTER = 0; // MIN filter
const TEXTURE_SMOOTH = false; // MAG filter
...
const EXTRA_SETTING_UNLOCK = 35; // true level - at effective level 52
const FINAL_UNLOCK = 59;         // effective level - redeemer - at true level 40
const FINAL_UNLOCK_TEXT = 60;    // effective level - at true level 41
...
if (LOG_GUNS) {
    ...
    for (const i of STATS.gun) {
        const damage = i.damage * (i.penetration || 1) * (i.bullets || 1);
        const r = v => String(Math.round(v * 10)).padStart(5);
        console.log(i.name.padEnd(17) + r(damage * i.shootSpeed) + r(damage * i.ammo));
    }
}
