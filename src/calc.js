...
function roundDecimals(v, d) {
    return v.toFixed(d).replace(/\.?0*$/g,"");
}
...
function length(x1, y1, x2, y2) {
    if (x2 !== undefined) return Math.hypot(x1 - x2, y1 - y2);
...
function rotationDistance(r1, r2) {
    ...
    const d1 = modulo(r2, PI2) - modulo(r1, PI2);
    const d2 = PI2 - Math.abs(d1);
...
function drawObjects(o, ...a) {
    for (const i of o) i.draw(...a);
}

function drawObjectsBloom(o, ...a) {
    for (const i of o) i.drawBloom(...a);
}
