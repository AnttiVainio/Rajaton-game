...
function Player(gl, shader, colorShader, textShader, LEVEL_PREV, TRUE_LEVEL, LEVEL, menu, inventory, world, fluid, keyboard, mouse) {
    const fadeBox = new BoxTex(gl, shader, "fade1").setAdditive();
    const buttonBox = new BoxTex(gl, shader, "button");
    const spaceBox = new BoxTex(gl, shader, "button_space");
    const arrowBox = new BoxTex(gl, shader, "arrow").setAdditive();
    const jarBox = new BoxTex(gl, shader, "jar");
    const jarfadeBox = new BoxTex(gl, shader, "jarfade").setAdditive();
    ...
    const gunStats = [];
    let minGun = 99, maxGun = 0, gunSelection, bestGun;
    for (const gun of STATS.gun) {
        if (gun.unlock !== undefined && gun.unlock <= LEVEL) {
            gunStats[gun.slot] = gun;
            minGun = Math.min(minGun, gun.slot);
            maxGun = Math.max(maxGun, gun.slot);
            bestGun = gun.slot;
            // force gun selection when a new gun is unlocked
            if (gun.unlock > LEVEL_PREV) gunSelection = gun.slot;
        }
    }
    if (gunSelection === undefined) gunSelection = inventory ? inventory.gunSelection : bestGun;
    let gun = gunStats[gunSelection];
    let gunText = 1;
    ...
    this.act = function(delta, mToGamex, mToGamey) {
        ...
        if (human.isActive()) {
            ...
            if (air < 80 && (
                ...
                keyboard.has("ControlLeft") ||
                keyboard.has("ControlRight") ||
                keyboard.has("Numpad0"))) {
                recharge = RECHARGE;
                rechargeNag = 0;
            }
            ...
            if (recharge <= 0) {
                if (keyboard.has("KeyQ") || keyboard.has("KeyE") || keyboard.has("wheelup") || keyboard.has("wheeldown")) {
                    if (!gunSelectLock) {
                        gunSelectLock = true;
                        if (keyboard.has("KeyQ") || keyboard.has("wheeldown")) {
                            gunSelection--;
                    ...
                    gun = gunStats[gunSelection];
                    gunText = 1;
                    human.setGun(gun);
                }
                else gunSelectLock = false;
                for (let i = 1; i < 10; i++) { // 1 - 9 but not 0
                    if (keyboard.has(`Digit${i}`) || keyboard.has(`Numpad${i}`)) {
                ...
                let up = false;
                if ((keyboard.has("KeyW") || keyboard.has("ArrowUp")) && air > 0) {
                    up = true;
                ...
                if (mimicStats || (mx && my)) {
                    mimicStats = [up,
                        keyboard.has("KeyD") || keyboard.has("ArrowRight"),
                        keyboard.has("KeyA") || keyboard.has("ArrowLeft"),
                        shoot,
    ...
    this.drawHud = function() {
        if (this.getHealth() > 0) {
            ...
            if (!world.getArena()) {
                ...
                if (p[0] > exitPos[0] * (TRUE_LEVEL - 1) * 0.1) {
            ...
            if (healthFlash > 0.01) {
                fadeBox.drawRot(0, -0.7, 0.3, 1, PI90, [1, 0, 0, Math.min(1, healthFlash * 1.5) * 0.55 - 0.01]);
                fadeBox.drawRot(0, 0.7, 0.3, 1, -PI90, [1, 0, 0, Math.min(1, healthFlash * 1.5) * 0.55 - 0.01]);
            }
            // Jars
            const JAR_SIZE = 0.85;
