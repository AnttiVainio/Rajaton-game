...
function Human(gl, shader, colorShader, spawnEffect, color, visorColor, stats, gun, world, x, y, enemyId = -1) {
    ...
    const frostBox = new BoxTex(gl, shader, "frost").setAdditive();
    ...
    let mimic = false;
    if (!stats.player) {
        if (!randInt(0, 25)) special = 1;
        if (special) {
            color = [0, 0, 0, 1];
            visorColor = [1.5, 1.5, 1.5, 1];
        }
        if (!randInt(0, 30)) special = 2;
        if (special === 2) {
            color = [3, 3, 3, 1];
            visorColor = [1.5, 0, 0, 1];
        }
        if (!special && !randInt(0, 40)) mimic = true;
    }
    ...
    this.act = function(delta, bullets, shakeFun, recharging, mx, my, up, right, left, shoot, air, blood, water, lava, mimicStats) {
        ...
        if (this.isActive()) {
            ...
            if (mx !== undefined && my !== undefined) {
                ...
                rec = (rot < 0 ? mix(0, PI, recoil, 0, Math.abs(rot * 2 + PI)) : mix(0, PI, -recoil, 0, Math.abs(rot * 2 - PI))) * RECOIL;
    ...
    this.draw = function() {
        if (this.isActive()) {
            ...
            const rot = mousex === undefined || mousey === undefined ? 0 :
                Math.atan2(x - mousex - targetOffset[0], mousey + targetOffset[1] - y) + PI * (flip ? -0.5 : 0.5);
    ...
    this.drawBloom = function() {
        if (this.isActive()) {
            ...
            const targetOffset = this.getTargetOffset();
            const rot = mousex === undefined || mousey === undefined ? 0 :
                Math.atan2(x - mousex - targetOffset[0], mousey + targetOffset[1] - y) + PI90;
