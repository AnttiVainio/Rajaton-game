...
function BoxTex(gl, shader, tex = null, func = null) {
    let texture = tex ? loadTexture(gl, tex, func) : null;
...
function Text(gl, shader) {
    ...
    this.draw = function(txt, x, y, sizey, widthMult, alphaOrColor = 1.0, align = "center") {
        if (checkPosition(x, y, Math.abs(sizey * TEXT_WIDTH * widthMult), Math.abs(sizey))) {
            useTextTexture(gl, txt, align);
            box.draw(x, y, sizey * TEXT_WIDTH * widthMult, sizey, alphaOrColor);
        }
    }

    this.drawRot = function(txt, x, y, sizey, widthMult, rot, alphaOrColor = 1.0, align = "center") {
        const size = Math.max(Math.abs(sizey * TEXT_WIDTH * widthMult), Math.abs(sizey));
        if (checkPosition(x, y, size, size)) {
            useTextTexture(gl, txt, align);
            box.drawRot(x, y, sizey * TEXT_WIDTH * widthMult, sizey, rot, alphaOrColor);
        }
    }
}
