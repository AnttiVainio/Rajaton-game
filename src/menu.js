...
function Button(active, _txt, x, y, _size, width, color = [1, 1, 1]) {
    ...
    this.hover = function(forceActive = false) {
        const retval = (active || forceActive) && _hover && life > 1;
        ...
        if (_volume && _hover && down) {
            ...
            if (vol0 !== vol) {
                if (_volume === 1) {
                    ...
                    localStorage.setItem("rajaton_soundVol", String(SOUND_VOL));
                }
                else {
                    setMusicVolume(vol);
                    localStorage.setItem("rajaton_musicVol", String(MUSIC_VOL));
                }
            }
        }
...
function Menu(gl, shader, colorShader, textShader, mouse) {
    /*SETTINGS.quality = localStorage.getItem("rajaton_quality");
    ...
    SETTINGS.showFPS = localStorage.getItem("rajaton_FPS");
    ...
    SETTINGS.enableShake = localStorage.getItem("rajaton_shake");
    ...
    SETTINGS.showHud = localStorage.getItem("rajaton_hud");
    ...
    SETTINGS.shaderOption = localStorage.getItem("rajaton_shader");
    ...
    SOUND_VOL = localStorage.getItem("rajaton_soundVol");
    ...
    MUSIC_VOL = localStorage.getItem("rajaton_musicVol");
    ...
    const targetBox = new BoxTex(gl, shader, "target2").setAdditive();
    const beam = new Beam(gl, colorShader).setAdditive();
    ...
    function removeButtons() {
        grace = 0.1;
        for (const i of buttons) i.remove();
    }

    this.getLevel = function() {
        return Number(localStorage.getItem("rajaton_level")) || 1;
    }

    this.saveLevel = function(level) {
        if (level > this.getLevel()) localStorage.setItem("rajaton_level", String(level));
    }

    this.setSettings = function(activate, fast = false) {
        if (activate) {
            substate = 1;
            removeButtons();
            if (fast) buttons.splice(0, buttons.length);
            const extraSetting = this.getLevel() >= EXTRA_SETTING_UNLOCK;
    ...
    this.setDifficulty = function(activate) {
        if (activate) {
            substate = 2;
            removeButtons();
            credits = [];
            ...
            let pos = 0;
            let i = 0;
            // Old system where every level didn't have a checkpoint
            /*let total = 0;
            while (i < levels) {
                total++;
                if (i === 0) i += 3;
    ...
    this.setMain = function(cb) {
        if (state !== 2) {
            ...
            const menuButtons = () => {
                buttons.splice(0, 0, new Button(false, FULLSCREEN_KEY.slice(-1) + " to fullscreen", 0, 0.6, 0.08, 1.3));
    ...
    this.act = function(delta) {
        ...
        const clickDifficulty = d => { return callbacks[0](d); };
        if (substate === 2 && difficultyButtons.length <= 1) return clickDifficulty(0);

        /*
         * Define what the menu buttons do
         */
        if (mouse[1]) down = true;
        else if (down) {
            down = false;
            if (substate === 1) { // settings
                if (buttons[0].hover()) {
                    SETTINGS.shaderOption = (SETTINGS.shaderOption + 1) % 2;
                    localStorage.setItem("rajaton_shader", SETTINGS.shaderOption.toString());
                    return this.setSettings(true, true);
                }
                if (buttons[1].hover()) {
                    SETTINGS.enableShake = !SETTINGS.enableShake;
                    localStorage.setItem("rajaton_shake", SETTINGS.enableShake ? "1" : "0");
                    return this.setSettings(true, true);
                }
                if (buttons[2].hover()) {
                    SETTINGS.showHud = !SETTINGS.showHud;
                    localStorage.setItem("rajaton_hud", SETTINGS.showHud ? "1" : "0");
                    return this.setSettings(true, true);
                }
                if (buttons[3].hover()) {
                    SETTINGS.showFPS = !SETTINGS.showFPS;
                    localStorage.setItem("rajaton_FPS", SETTINGS.showFPS ? "1" : "0");
                    return this.setSettings(true, true);
                }
                if (buttons[6].hover()) return this.setSettings(false);
            }
            else if (substate === 2) { // difficulty
                ...
                if (buttons[1].hover()) return this.setSettings(true);
                if (buttons[2].hover(true)) {
                    if (!document.fullscreenElement) document.getElementById("glCanvas").requestFullscreen();
